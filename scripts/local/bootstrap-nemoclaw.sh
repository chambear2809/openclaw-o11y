...
if ! command -v openshell >/dev/null 2>&1; then
  ...
  env \
    ...
    bash "${nemoclaw_dir}/scripts/install-openshell.sh"
  export PATH="${HOME}/.local/bin:${PATH}"
fi
...
env \
  ...
  NEMOCLAW_MODEL="${bootstrap_model}" \
  NEMOCLAW_POLICY_MODE=suggested \
  NEMOCLAW_RECREATE_SANDBOX="$([[ "${recreate_sandbox}" == "true" ]] && printf '1' || printf '0')" \
...
echo "Local NemoClaw/OpenShell sandbox is running"
